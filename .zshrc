...
alias pdotfiles='cd ~/dotfiles && git add . && if [ $(git commit -m "update" && git push -u origin main) -eq 0 ]; then cd -; fi && cd -'
alias se="sudoedit"
alias cdotfiles="cd $HOME/dotfiles"
alias cnvim="cd $HOME/dotfiles/.config/nvim"
...
alias wezwin="bash ~/dotfiles/installation-scripts/kiss-installation/install-windows-configs.sh"
...
alias oc=opencode
alias oc-heavy='OPENCODE_PROFILE=heavy rf && opencode'
...
# fnm (Fast Node Manager) — replaces nvm. --use-on-cd auto-switches Node on cd via .nvmrc/.node-version.
